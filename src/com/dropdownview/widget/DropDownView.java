 ...
 * 
 * Modifications by: Garrett Franks
 */
...
public class DropDownView extends ViewGroup {
	 ...
	 */
	public static interface OnDropDownViewShowListener {
	 ...
	 */
	public static interface OnDropDownViewDismissListener {
	 ...
	 */
	public static interface OnDropDownViewScrollListener {
	...
	/**
	 * Creates a new DropDownView from a specified set of attributes defined in
	 * XML.
	 * 
	 * @param context
	...
	public DropDownView( Context context, AttributeSet attrs, int defStyle )
	{
		...
		TypedArray a = context.obtainStyledAttributes( attrs, R.styleable.DropDownView, defStyle, 0 );
		
		int orientation = a.getInt( R.styleable.DropDownView_direction, ORIENTATION_BTT );
		mVertical = ( orientation == ORIENTATION_BTT || orientation == ORIENTATION_TTB );
		mBottomOffset = (int)a.getDimension( R.styleable.DropDownView_bottomOffset, 0.0f );
		mTopOffset = (int)a.getDimension( R.styleable.DropDownView_topOffset, 0.0f );
		mAllowSingleTap = a.getBoolean( R.styleable.DropDownView_allowSingleTap, true );
		mAnimateOnClick = a.getBoolean( R.styleable.DropDownView_animateOnClick, true );
		mInvert = ( orientation == ORIENTATION_TTB || orientation == ORIENTATION_LTR );
		
		int handleId = a.getResourceId( R.styleable.DropDownView_handle, 0 );
		if ( handleId == 0 ) { throw new IllegalArgumentException( "The handle attribute is required and must refer "
				+ "to a valid child." ); }
		
		int contentId = a.getResourceId( R.styleable.DropDownView_content, 0 );
	...
	protected void onFinishInflate()
	{
		...
		mHandle.setOnClickListener( new DropDownViewToggler() );
	...
	protected void onMeasure( int widthMeasureSpec, int heightMeasureSpec )
	{
		...
		if ( widthSpecMode == MeasureSpec.UNSPECIFIED || heightSpecMode == MeasureSpec.UNSPECIFIED ) { throw new RuntimeException(
				"DropDownView cannot have UNSPECIFIED dimensions" ); }
	...
	public boolean onInterceptTouchEvent( MotionEvent event )
	{
		...
		if ( action == MotionEvent.ACTION_DOWN ) {
			...
			if ( mOnDropDownViewScrollListener != null ) {
				mOnDropDownViewScrollListener.onScrollStarted();
			}
	...
	private void stopTracking()
	{
		...
		if ( mOnDropDownViewScrollListener != null ) {
			mOnDropDownViewScrollListener.onScrollEnded();
		}
	...
	{
		prepareContent();
		final OnDropDownViewScrollListener scrollListener = mOnDropDownViewScrollListener;
	...
	private void dismissDropDownView()
	{
		...
		mExpanded = false;
		if ( mOnDropDownViewDismissListener != null ) {
			mOnDropDownViewDismissListener.onDropDownViewDismissed();
		}
	}
	...
	private void showDropDownView()
	{
		...
		if ( mOnDropDownViewShowListener != null ) {
			mOnDropDownViewShowListener.onDropDownViewShown();
		}
	}
	 ...
	 * 
	 * @param onDropDownViewScrollListener
	 *           The listener to be notified when scrolling starts or stops.
	 */
	public void setOnDropDownViewScrollListener( OnDropDownViewScrollListener onDropDownViewScrollListener )
	{
		mOnDropDownViewScrollListener = onDropDownViewScrollListener;
	}
	...
	/**
	 * Unlocks the DropDownView so that touch events are processed.
	 * 
	 * @see #lock()
	 */
	public void unlock()
	...
	/**
	 * Locks the DropDownView so that touch events are ignores.
	 * 
	 * @see #unlock()
	 */
	public void lock()
	...
	private class DropDownViewToggler implements OnClickListener {
